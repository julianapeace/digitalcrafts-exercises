...
--
--Based on the previous query, get the count of the number of techs used by each project. COME BACK LATER**** COMPLETED
--
...
FROM
  project_uses_tech
INNER JOIN project
  ON project.id = project_uses_tech.project_id GROUP BY project.id;
...
--
--List all the distinct techs that are used by at least one project. Ruby and JavaScript and Java should not be in here. 
--
...
FULL OUTER JOIN tech ON project_uses_tech.tech_id = tech.id WHERE project.name IS NULL;

--
--List all the distinct projects that use at least one tech. Whiteboard Exercises shoudl not be in here. 
--
SELECT DISTINCT(project.name) FROM project
FULL OUTER JOIN project_uses_tech ON project_uses_tech.project_id = project.id
FULL OUTER JOIN tech ON project_uses_tech.tech_id = tech.id WHERE tech_id IS NOT NULL;

--
-- List all the distinct projects that use no tech. Only Whiteboard Exercises should be in here. 
--
SELECT DISTINCT(project.name) FROM project
FULL OUTER JOIN project_uses_tech ON project_uses_tech.project_id = project.id
FULL OUTER JOIN tech ON project_uses_tech.tech_id = tech.id WHERE tech.id IS NULL;
--
--Order the projects by how many tech it uses.COME BACK LATER. ****COMPLETED****
--
SELECT 
  project.id,
  project.name,
  count(project_uses_tech.tech_id) as mycount
FROM
  project_uses_tech
INNER JOIN project
  ON project.id = project_uses_tech.project_id GROUP BY project.id, project.name ORDER BY mycount DESC;

--
--Order the tech by how many projects use it. COME BACK LATER. ****COMPLETED****
--
  
SELECT tech.id,
tech.name, 
count(project_uses_tech.project_id) as mycount
FROM tech
INNER JOIN project_uses_tech
ON project_uses_tech.tech_id = tech.id GROUP BY tech.id, tech.name ORDER BY mycount DESC;
...
FROM tech
INNER JOIN project_uses_tech
ON project_uses_tech.tech_id = tech.id GROUP BY tech.id, tech.name) AS tech_count;
